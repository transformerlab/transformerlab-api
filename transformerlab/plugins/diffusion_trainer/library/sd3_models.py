...
class SingleDiTBlock(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs,
        ...
        self.attn = AttentionLinears(
            dim=hidden_size, num_heads=num_heads, qkv_bias=qkv_bias, pre_only=pre_only, qk_norm=qk_norm
        )
        ...
        if self.x_block_self_attn:
            ...
            self.attn2 = AttentionLinears(
                dim=hidden_size, num_heads=num_heads, qkv_bias=qkv_bias, pre_only=False, qk_norm=qk_norm
            )
    ...
    def post_attention_x(
        self, attn, attn2, x, gate_msa, shift_mlp, scale_mlp, gate_mlp, gate_msa2, attn1_dropout: float = 0.0
    ):
        assert not self.pre_only
    ...
    def enable_scaled_pos_embed(self, use_scaled_pos_embed: bool, latent_sizes: Optional[list[int]]):
        ...
        if self.use_scaled_pos_embed:
            ...
            self.resolution_area_to_latent_size = [
                (area, latent_size) for area, latent_size in zip(max_areas, patched_sizes)
            ]
