...
class DoubleStreamBlock(nn.Module):
    ...
    def _forward(
        ...
        if txt_attention_mask is not None:
            ...
            attn_mask = torch.cat(
                (attn_mask, torch.ones(attn_mask.shape[0], img.shape[1], device=attn_mask.device, dtype=torch.bool)),
                dim=1,
            )  # b, seq_len + img_len
...
class Flux(nn.Module):
    ...
    def enable_block_swap(self, num_blocks: int, device: torch.device):
        ...
        assert (
            double_blocks_to_swap <= self.num_double_blocks - 2 and single_blocks_to_swap <= self.num_single_blocks - 2
        ), (
