...
def sample_image_inference(
    ...
    if cfg_scale != 1.0:
        ...
        neg_t5_attn_mask = (
            neg_t5_attn_mask.to(accelerator.device)
            if args.apply_t5_attn_mask and neg_t5_attn_mask is not None
            else None
        )
...
def add_flux_train_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--ae", type=str, help="path to ae (*.sft or *.safetensors) / aeのパス（*.sftまたは*.safetensors）"
    )
    parser.add_argument(
