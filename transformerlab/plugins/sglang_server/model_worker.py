...
def get_assistant_tokens(conv, tokenizer=None):
    """
    Dynamically infer assistant start/end tokens from the prompt generated by ConversationTemplate.
    Avoids hardcoding SeparatorStyle cases.
    """
    # Generate a dummy prompt with both user and assistant turns
    conv.messages = []
    conv.append_message(conv.roles[0], "Hello")
    conv.append_message(conv.roles[1], "Hi! How can I help you today?")
    prompt = conv.get_prompt()

    # Try to match the last assistant reply
    # This assumes assistant response is at the end of the prompt
    assistant_role = conv.roles[1]
    pattern = re.compile(
        rf"(?P<start>[^\n]*{re.escape(assistant_role)}[^\n]*?[:：>\|]?)\s*(?P<content>.*?)(?P<end>\n|$)", re.S | re.I
    )

    matches = list(pattern.finditer(prompt))
    if not matches:
        raise ValueError("Could not identify assistant markers in prompt.")

    last_match = matches[-1]
    start_text = last_match.group("start").strip()
    end_text = last_match.group("end").strip() if last_match.group("end").strip() else None

    # Normalize with tokenizer if available
    if tokenizer:
        start_token = tokenizer.convert_tokens_to_string(tokenizer.tokenize(start_text)).strip()
        if end_text:
            end_token = tokenizer.convert_tokens_to_string(tokenizer.tokenize(end_text)).strip()
        else:
    ...
    # Build regex to extract latest assistant block
    if end_token:
