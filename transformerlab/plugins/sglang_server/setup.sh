...
if [ "$BACKEND" = "rocm" ]; then
    echo "[setup] Installing ROCm-compatible PyTorch (assumes ROCm 6.3)"
    uv pip install torch torchvision --index "https://download.pytorch.org/whl/rocm6.3"
else
    echo "[setup] Installing CUDA 12.8-compatible PyTorch"
    uv pip install torch torchvision --index "https://download.pytorch.org/whl/cu128"
fi
