...
# Ensure we have configuration parameters
print("Input config:")
...
steps_per_eval = str(config.get("steps_per_eval", "200"))
...
try:
    ...
    print(error_msg)
    job.set_job_completion_status("failed", "Could not load dataset.")
    sys.exit(1)
...
popen_command = [
    ...
    data_directory,
    "--steps-per-report",
    config.get("steps_per_report", "10"),
    "--steps-per-eval",
    steps_per_eval,
    "--save-every",
    config.get("save_every", "100"),
]
