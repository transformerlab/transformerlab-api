...
async def dataset_get(dataset_id: str, split: str = "train"):
    ...
    try:
        if d["location"] == "local":
            dataset = load_dataset(path=dirs.dataset_dir_by_id(dataset_id), streaming=False)
        else:
            dataset_config = d.get("json_data", {}).get("dataset_config", None)
            if dataset_config is not None:
                dataset = load_dataset(dataset_id, dataset_config, trust_remote_code=True, streaming=False)
            else:
                dataset = load_dataset(dataset_id, trust_remote_code=True, streaming=False)
    except Exception as e:
        error_msg = f"{type(e).__name__}: {e}"
        return {"status": "error", "message": error_msg}
    ...
    result["splits"] = list(dataset.keys())
    result["file_location"] = dirs.dataset_dir_by_id(dataset_id) if d["location"] == "local" else None
    result["len"] = dataset_len
    return {"status": "success", "data": result}
