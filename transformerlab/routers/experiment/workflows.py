...
async def cancel_workflow_run(workflow_run_id: str, experimentId: int):
    ...
    if not isinstance(workflow_run.get("current_job_ids", "[]"), list) or not isinstance(
        workflow_run.get("current_job_ids", "[]"), dict
    ):
...
async def load_workflow_context(active_run):
    ...
    if not isinstance(active_run.get("current_tasks", "[]"), list) or not isinstance(
        active_run.get("current_tasks", "[]"), dict
    ):
    ...
    # List of job IDs
    if not isinstance(active_run.get("current_job_ids", "[]"), list) or not isinstance(
        active_run.get("current_job_ids", "[]"), dict
    ):
...
def prepare_next_task_io(task_def: dict, previous_outputs: dict):
    ...
    if task_type == "EVAL":
        ...
        # Map relevant outputs to specific input fields
        for key in ["model_name", "model_architecture", "adaptor_name"]:
            if key in previous_outputs:
                inputs[key] = previous_outputs[key]
        # Generate dynamic output fields if not provided
        if "dataset_name" not in inputs:
            outputs["dataset_name"] = str(uuid.uuid4()).replace("-", "")
