...
def parse_cpu_metrics(powermetrics_parse):
    ...
    for cluster in cpu_clusters:
        ...
        for cpu in cluster["cpus"]:
            ...
            core.append(cpu["cpu"])
            cpu_metric_dict[name + str(cpu["cpu"]) +
                            "_freq_Mhz"] = int(cpu["freq_hz"] / (1e6))
            cpu_metric_dict[name + str(cpu["cpu"]) +
                            "_active"] = int((1 - cpu["idle_ratio"]) * 100)
    cpu_metric_dict["e_core"] = e_core
    cpu_metric_dict["p_core"] = p_core

    # If a field exists called: E-Cluster0_active:
    if "E-Cluster0_active" in cpu_metric_dict:
