...
def setup_device_map(config):
    ...
    for i in range(num_gpus):
        try:
            ...
            print(f"GPU {i}: Total {total_memory:.1f}GB, Allocated {allocated_memory:.1f}GB, Reserved {reserved_memory:.1f}GB, Free {free_memory:.1f}GB, Usage {usage_percent:.1f}% [HEALTHY]")
...
def load_pipeline_with_device_map(model_path, adaptor_path, is_img2img, is_inpainting, device):
    ...
    if device == "auto":
        ...
        if total_available_memory < 12:
            print(f"Warning: Only {total_available_memory}GB available across all GPUs. FLUX models typically need 12-24GB.")
    ...
    try:
        ...
        if torch.cuda.is_available():
            for i in range(torch.cuda.device_count()):
                ...
                print(f"GPU {i} after loading: Allocated {allocated:.1f}GB, Reserved {reserved:.1f}GB, Total {total:.1f}GB")
        ...
        try:
            ...
            if "illegal memory access" in str(e) or "CUDA error" in str(e):
                ...
                raise RuntimeError(f"CUDA illegal memory access: {str(e)}. "
            ...
            if torch.cuda.is_available():
                ...
                for i in range(torch.cuda.device_count()):
                    try:
                        ...
                        print(f"GPU {i} ({props.name}): Allocated {allocated:.1f}GB, Reserved {reserved:.1f}GB, Total {total:.1f}GB")
        ...
        if torch.cuda.is_available():
            for i in range(torch.cuda.device_count()):
                ...
                free_memory = total_memory - reserved_memory
                print(f"GPU {i} Memory Status: Total {total_memory:.1f}GB, Allocated {allocated_memory:.1f}GB, Reserved {reserved_memory:.1f}GB, Free {free_memory:.1f}GB")
