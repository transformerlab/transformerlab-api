...
def test_gguf_model_without_config_detection():
...
def test_download_gguf_file_invalid_filename():
    """Test downloading with invalid filename"""
    with TestClient(app) as client:
        response = client.get(
            "/model/download_gguf_file?model=MaziyarPanahi/gemma-3-1b-it-GGUF&filename=nonexistent.gguf"
        )
...
def test_download_gguf_file_invalid_model():
...
def test_regular_model_still_works():
...
def test_model_gallery_includes_gguf():
...
def test_gguf_download_with_job_id():
...
def test_gguf_detection_edge_cases():
...
def test_gguf_large_model_file_listing():
